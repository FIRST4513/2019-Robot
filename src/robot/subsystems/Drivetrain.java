...
public class Drivetrain extends Subsystem {
    ...
    private DigitalInput climbExtendSwitch;
    ...
    */
    ...
    private static final double DRIVE_VOLT_RAMP_RATE = 0.15;        // Ramp Volt change ramp rate to 100 milliseconds 

    private static final int DRIVE_MTR_MAX_PEAK_CURRENT = 40;    // Amps
    private static final int DRIVE_MTR_MAX_PEAK_DURATION = 0;   // Milliseconds
    private static final int DRIVE_MTR_MAX_CONT_CURRENT = 40;    // Amps
    private static final boolean DRIVE_MTR_CURRENT_LIMIT_FLAG = true;

    private static final double HIGH_OVERALL_CURRENT = 100.0;
    private static final double LOW_VOLT_LEVEL = 8.0;
    private static final double HIGH_SRX_CURRENT = 25.0;
    private static final double HIGH_PDP_CURRENT = 25.0;
    ...
    public Drivetrain() {
        ...
        climbExtendSwitch = new DigitalInput(21);
        addChild("ClimbExtendSwitch",climbExtendSwitch);
    ...
    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTORS
        ...
        leftFMtr.configOpenloopRamp(DRIVE_VOLT_RAMP_RATE);                  // Voltage TRamp Rate limiter
        ...
        leftRMtr.configOpenloopRamp(DRIVE_VOLT_RAMP_RATE);                  // Voltage TRamp Rate limiter
        ...
        leftTMtr.configOpenloopRamp(DRIVE_VOLT_RAMP_RATE);                  // Voltage TRamp Rate limiter
        ...
        rightFMtr.configOpenloopRamp(DRIVE_VOLT_RAMP_RATE);                  // Voltage TRamp Rate limiter
        ...
        rightRMtr.configOpenloopRamp(DRIVE_VOLT_RAMP_RATE);                  // Voltage TRamp Rate limiter
        ...
        rightTMtr.configOpenloopRamp(DRIVE_VOLT_RAMP_RATE);                  // Voltage TRamp Rate limiter
    ...
    void calcStats() {
        ...
        //update_Pwr_Stats();
        //check_Need_To_Log_Currents();
    }

    private void update_Pwr_Stats(){
        /*
        leftStats.PDP_currentBMtr = Robot.pdp.getCurrent(LEFT_BACK_MOTOR_BRKR);     // Left Back Motor
        leftStats.SRX_currentBMtr = leftRMtr.getOutputCurrent();
        
        leftStats.PDP_currentTMtr = Robot.pdp.getCurrent(LEFT_TOP_MOTOR_BRKR);      // Left Top Motor
        leftStats.SRX_currentTMtr = leftTMtr.getOutputCurrent();

        leftStats.PDP_currentFMtr = Robot.pdp.getCurrent(LEFT_FRONT_MOTOR_BRKR);    // LeftFront Motor
        leftStats.SRX_currentFMtr = leftFMtr.getOutputCurrent();
        
        rightStats.PDP_currentBMtr = Robot.pdp.getCurrent(RIGHT_BACK_MOTOR_BRKR);   // Right Back Motor
        rightStats.SRX_currentBMtr = rightRMtr.getOutputCurrent();
        
        rightStats.PDP_currentTMtr = Robot.pdp.getCurrent(RIGHT_TOP_MOTOR_BRKR);    // Right Top Motor
        rightStats.SRX_currentTMtr = rightTMtr.getOutputCurrent();

        rightStats.PDP_currentFMtr = Robot.pdp.getCurrent(RIGHT_FRONT_MOTOR_BRKR);  // Right Front Motor
        rightStats.SRX_currentFMtr = rightFMtr.getOutputCurrent();

        avgStats.current = Robot.pdp.getTotalCurrent();                             // Overal Current
        avgStats.voltage = Robot.pdp.getVoltage();                                  // Overal Voltage
        */
    }

    private void check_Need_To_Log_Currents(){
        String logmsg;
        if  ((avgStats.current > HIGH_OVERALL_CURRENT) ||
            (avgStats.voltage < LOW_VOLT_LEVEL )) {
                logmsg = String.format("Power Overload Overall Data,Voltage=,%g,Current=,%g", 
                                avgStats.voltage, avgStats.current);
                Robot.logger.appendLog(logmsg);
            }

        if  ((leftStats.SRX_currentBMtr > HIGH_SRX_CURRENT) ||
            (leftStats.SRX_currentTMtr > HIGH_SRX_CURRENT) || 
            (leftStats.SRX_currentFMtr > HIGH_SRX_CURRENT)  ||
            (rightStats.SRX_currentBMtr > HIGH_SRX_CURRENT) ||
            (rightStats.SRX_currentTMtr > HIGH_SRX_CURRENT) ||
            (rightStats.SRX_currentFMtr > HIGH_SRX_CURRENT)) {
                logmsg = String.format("Power Overload SRX Data,LBMTR=,%g,LTMTR=,%g,LFMTR=,%g,RBMTR=,%g,RTMTR=,%g,RFMTR=,%g,",
                                        leftStats.SRX_currentBMtr, leftStats.SRX_currentTMtr, leftStats.SRX_currentFMtr,
                                        rightStats.SRX_currentBMtr, rightStats.SRX_currentTMtr, rightStats.SRX_currentFMtr);
                Robot.logger.appendLog(logmsg);
            }

        if  ((leftStats.PDP_currentBMtr > HIGH_PDP_CURRENT) ||
            (leftStats.PDP_currentTMtr > HIGH_PDP_CURRENT) || 
            (leftStats.PDP_currentFMtr > HIGH_PDP_CURRENT)  ||
            (rightStats.PDP_currentBMtr > HIGH_PDP_CURRENT) ||
            (rightStats.PDP_currentTMtr > HIGH_PDP_CURRENT) ||
            (rightStats.PDP_currentFMtr > HIGH_PDP_CURRENT)) {
                logmsg = String.format("Power Overload PDP Data,LBMTR=,%g,LTMTR=,%g,LFMTR=,%g,RBMTR=,%g,RTMTR=,%g,RFMTR=,%g,",
                                        leftStats.PDP_currentBMtr, leftStats.PDP_currentTMtr, leftStats.PDP_currentFMtr,
                                        rightStats.PDP_currentBMtr, rightStats.PDP_currentTMtr, rightStats.PDP_currentFMtr);
                Robot.logger.appendLog(logmsg);
            }
    }
    ...
    void update_SmartDashboard() {
        ...
        SmartDashboard.putNumber("Left_F_Mtr Curr",Rmath.mRound(leftStats.PDP_currentFMtr, 3));
        SmartDashboard.putNumber("Left_B_Mtr Curr",Rmath.mRound(leftStats.PDP_currentBMtr, 3));
        SmartDashboard.putNumber("Left_T_Mtr Curr",Rmath.mRound(leftStats.PDP_currentTMtr, 3));

        SmartDashboard.putNumber("Right_F_Mtr Curr",Rmath.mRound(rightStats.PDP_currentFMtr, 3));
        SmartDashboard.putNumber("Right_B_Mtr Curr",Rmath.mRound(rightStats.PDP_currentBMtr, 3));
        SmartDashboard.putNumber("Right_T_Mtr Curr",Rmath.mRound(rightStats.PDP_currentTMtr, 3));

        SmartDashboard.putNumber("Avg_Mtr Curr",Rmath.mRound(avgStats.PDP_currentAVG, 3));
    ...
    public void logDrivetrain() {
        ...
        line += ",LeftTFB=," + Rmath.mRound(leftStats.PDP_currentTMtr, 3) +
                         "," + Rmath.mRound(leftStats.PDP_currentFMtr, 3) +
                         "," + Rmath.mRound(leftStats.PDP_currentBMtr, 3);
        line += ",RightTFB=," + Rmath.mRound(rightStats.PDP_currentTMtr, 3) +
                        "," + Rmath.mRound(rightStats.PDP_currentFMtr, 3) +
                        "," + Rmath.mRound(rightStats.PDP_currentBMtr, 3);
        line += ",Avg Cur&Volt=," + Rmath.mRound(avgStats.PDP_currentAVG, 3) +
    ...
    private class DriveStats {
        ...
        double PDP_currentFMtr, PDP_currentTMtr, PDP_currentBMtr, PDP_currentAVG; 
        double SRX_currentFMtr, SRX_currentTMtr, SRX_currentBMtr, SRX_currentAVG; 
        ...
        public void clearStats() {
            ...
            voltage = 0;
            current = 0;
            PDP_currentFMtr =  PDP_currentTMtr = PDP_currentBMtr = PDP_currentAVG = 0;
            SRX_currentFMtr =  SRX_currentTMtr = SRX_currentBMtr = SRX_currentAVG = 0;
        }
    }
}
